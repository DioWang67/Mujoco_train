...
Sedon is currently in a locomotion research phase. The practical goal is not
...
`code/current`, and runs a remote Sedon smoke check. Password-based deployment
...
Use this to resume yesterday's Sedon training from the newest remote checkpoint:
...
```text
/root/anaconda3/mujoco-train-system/runs/sedon/models/sedon/latest_model.zip
/root/anaconda3/mujoco-train-system/runs/sedon/models/sedon/vecnorm.pkl
```
...
mkdir -p /root/anaconda3/mujoco-train-system/runs/sedon/logs/sedon
...
export MUJOCO_TRAIN_PROJECT_SLUG=sedon
...
python -m sedon_baseline.train --total-timesteps 2000000 --n-envs 128 --reset-noise-scale 0.005 2>&1 | tee /root/anaconda3/mujoco-train-system/runs/sedon/logs/sedon/blue_dynamic_support_gait_2m.log
```

Use `python -m sedon_baseline.train` on the remote if the generic dispatcher
behaves unexpectedly. It removes one layer of indirection and calls the Sedon
...
```bash
python -m sedon_baseline.train --total-timesteps 20000 --n-envs 32 --reset-noise-scale 0.01
```
...
```bash
tail -f /root/anaconda3/mujoco-train-system/runs/sedon/logs/sedon/resume_blue_dynamic_support_gait_*.log
```

TensorBoard from local Windows:

```powershell
scripts\sedon_tensorboard.bat
```
...
```powershell
python -m tools.debug_sedon_gait_viewer --mode policy --checkpoint-path models\sedon\latest_model.zip --vecnorm-path models\sedon\vecnorm.pkl --steps 600 --speed 0.5 --out-csv artifacts\sedon_debug\policy_gait_view.csv
```
...
```powershell
python -m tools.debug_sedon_pose_editor --scene private_assets/sedon/training_scene.xml
python -m tools.debug_sedon_gait_viewer --mode scripted --gait-seed-path artifacts/sedon_debug/sedon_reference_gait_seed.json --steps 400
```
...
```bash
python -m sedon_baseline.train --total-timesteps 10000000 --n-envs 32 --reset-noise-scale 0.01
```
...
- The code expects the newer Sedon training scene, but the active release is
