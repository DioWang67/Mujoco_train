# Seedon Blue-Like Dynamic Gait Spec
...
This document defines an experimental locomotion target and audit criteria for Seedon MuJoCo work. It is not a production API contract and should stay practical: precise enough to prevent false positives, but not so abstract that it slows mechanical diagnosis.
...
Seedon should gradually approach this gait pattern:
...
This is not final walking. Phase 0 is useful because it proves Seedon can stay upright, produce low-speed forward displacement, and expose load/contact diagnostics. It must not be reported as Blue-like dynamic gait.
...
- Verify that Seedon can use forward momentum and the rounded sole without losing stability.
