# Seedon v5_22 MuJoCo Migration

Task class: Class C prototype migration. This keeps the older Seedon MuJoCo
assets as archived reference and makes the updated `SEEDON_URDF_5_22` package
the active mechanical source for new Seedon diagnostics.

## Active Source

- Version: `v5_22`
- Source URDF: `private_assets/SEEDON_URDF_5_22/urdf/SEEDON_URDF_5_21.urdf`
- Generated scene: `private_assets/seedon_v5_22/training_scene.xml`
- Generator: `tools/seedon/geometry/generate_seedon_v5_22_mjcf_scene.py`
- Version registry: `configs/seedon/seedon_model_versions.yaml`
- Default Seedon env scene: `private_assets/seedon_v5_22/training_scene.xml`

## What Changed

- Uses the updated v5_22 STL visual meshes directly.
- Uses v5_22 mass, COM, inertia, joint origins, joint axes, and joint ranges.
- Keeps the old `private_assets/seedon/` scene as archived reference only.
- Adds a versioned output root so future versions can use
  `private_assets/seedon_<version>/` without changing the Seedon baseline code.
...
- `motor_ctrlrange=-100 100`
  - source: `assumption`
  - confidence: `low`
  - reason: no verified Seedon motor/controller spec was found.
...
- right-leg joint name adapter:
  ...
  - source: `assumption`
  - confidence: `low`
  - reason: existing Seedon env expects knee/ankle pitch names.
...
```powershell
python -B -m py_compile tools\seedon\geometry\generate_seedon_v5_22_mjcf_scene.py
python -B tools\seedon\geometry\generate_seedon_v5_22_mjcf_scene.py
python -B -m seedon_baseline.eval --episodes 1 --scene-path private_assets\seedon_v5_22\training_scene.xml --record
```
...
reproducibility while keeping new default Seedon work on `v5_22`.
