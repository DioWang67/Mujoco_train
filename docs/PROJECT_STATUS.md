...
- [x] Numeric BASE-vs-DR compare tool (`tools/compare_eval.py`).
- [x] Multi-seed aggregate compare with 95% CI (`tools/aggregate_compare.py`).
- [x] Release gate checker for single + aggregate reports (`tools/gate_check.py`).
- [x] Benchmark matrix runner with unified report outputs (`tools/benchmark_matrix.py`).
...
1. Add CI integration to block merges/releases when `tools/gate_check.py` fails under `preprod` profile.
2. Add terrain-specific scene variants (heightfield/ramp/step XML) and plug them into `configs/benchmark_matrix.json`.
