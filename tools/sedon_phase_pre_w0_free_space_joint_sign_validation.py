"""Phase Pre-W0 free-space joint sign validation for Sedon.

This Class C diagnostic validates Sedon leg joint sign effects without ground
...
DEFAULT_M0_SIGN_MAPPING = DEBUG_OUT_DIR / "phase_m0_duck_morphology_audit" / "sedon_joint_sign_mapping.json"
...
def infer_side(name: str) -> str:
    """Infer left/right side from common Sedon naming patterns."""
...
def leg_actuator_joint_pairs(model: mujoco.MjModel) -> list[tuple[int, int]]:
    """Return actuator/joint pairs that look like Sedon leg joints."""
...
def main() -> None:
    ...
    write_csv(
        config.output_dir / "sedon_free_space_joint_effect_probe.csv",
        rows,
        [
            "actuator_name",
            "joint_name",
    ...
    (config.output_dir / "sedon_free_space_joint_sign_mapping.json").write_text(
        json.dumps(summary, indent=2) + "\n",
        encoding="utf-8",
    )
    write_report(config.output_dir / "sedon_free_space_joint_sign_validation_report.md", config, summary)

    print(f"effect_probe={config.output_dir / 'sedon_free_space_joint_effect_probe.csv'}")
    print(f"mapping={config.output_dir / 'sedon_free_space_joint_sign_mapping.json'}")
    print(f"report={config.output_dir / 'sedon_free_space_joint_sign_validation_report.md'}")
