"""Sweep Sedon preload hip-roll targets for Blue-like in-place load ratios."""
...
DEFAULT_OUT_CSV = DEBUG_OUT_DIR / "sedon_preload_sweep.csv"
...
def _foot_world_z_forces(env: SedonStandingEnv) -> tuple[float, float, float]:
...
def _build_target(env: SedonStandingEnv, side: str, hip_roll: float, lean_roll: float) -> np.ndarray:
...
def _collect_frame_metrics(env: SedonStandingEnv) -> FrameContactMetrics:
...
def build_parser() -> argparse.ArgumentParser:
    """Build CLI parser for the Sedon preload sweep."""
