"""Phase W0-DuckRef conservative scripted walking smoke test for Sedon.
...
DEFAULT_DUCK_SCALED_REFERENCE = DEBUG_OUT_DIR / "phase_m0_duck_morphology_audit" / "sedon_duck_scaled_gait_reference.json"
DEFAULT_JOINT_SIGN_MAPPING = DEBUG_OUT_DIR / "phase_m0_duck_morphology_audit" / "sedon_joint_sign_mapping.json"
...
def side_for_geom(name: str) -> str:
    """Infer left/right side from a Sedon foot geom name."""
...
def foot_contact_forces(env: SedonStandingEnv) -> dict[str, float | bool | int]:
...
def foot_positions(env: SedonStandingEnv) -> dict[str, tuple[float, float, float]]:
...
def run_trial(config: SmokeConfig, params: TrialParams) -> tuple[list[dict[str, Any]], TrialSummary]:
    ...
    env = SedonStandingEnv(scene_path=config.scene_path, reset_noise_scale=0.0, reward_config=load_sedon_config_from_env())
...
def append_progress_log(path: Path, summary_payload: dict[str, Any]) -> None:
    """Append W0 result to the Sedon progress log."""
