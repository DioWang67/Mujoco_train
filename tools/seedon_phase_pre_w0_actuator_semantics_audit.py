"""Phase Pre-W0 Seedon actuator semantics audit.

This Class C diagnostic inspects MuJoCo actuator metadata before any
W0-DuckRef scripted walking smoke test. It answers whether Seedon controls look
...
from tools.seedon_debug_common import DEBUG_OUT_DIR, require_scene
...
def actuator_rows(model: mujoco.MjModel) -> list[dict[str, Any]]:
    ...
    Returns:
        Rows for `seedon_actuator_semantics_inventory.csv`.
    """
...
def main() -> None:
    ...
    write_csv(
        config.output_dir / "seedon_actuator_semantics_inventory.csv",
        rows,
        [
            "actuator_id",
            "actuator_name",
    ...
    (config.output_dir / "seedon_actuator_semantics_summary.json").write_text(
        json.dumps(summary, indent=2) + "\n",
        encoding="utf-8",
    )
    write_report(config.output_dir / "seedon_actuator_semantics_report.md", config, summary)

    print(f"inventory={config.output_dir / 'seedon_actuator_semantics_inventory.csv'}")
    print(f"summary={config.output_dir / 'seedon_actuator_semantics_summary.json'}")
    print(f"report={config.output_dir / 'seedon_actuator_semantics_report.md'}")
