"""Validate Sedon hip-roll model structure, kinematics, and dynamics."""
...
URDF_PATH = Path("private_assets/sedon/mjcf_source/sedon.urdf")
MJCF_PATH = Path("private_assets/sedon/training_scene.xml")
...
def _floor_contact_flags(env: SedonStandingEnv) -> dict[str, bool]:
...
def _body_world_position(env: SedonStandingEnv, body_name: str) -> tuple[float, float, float]:
...
def _geom_world_position(env: SedonStandingEnv, geom_name: str) -> tuple[float, float, float]:
...
def _foot_collision_mirror_summary(env: SedonStandingEnv) -> dict[str, str]:
...
def _write_report(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append("# Sedon Hip-Roll Model Validation")
    lines.append("")
