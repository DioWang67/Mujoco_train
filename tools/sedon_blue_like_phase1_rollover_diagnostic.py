"""Phase 1 controlled-falling / rollover diagnostic for Sedon.

This Class C diagnostic does not train PPO and does not modify scenes. It runs
a deterministic MuJoCo rollout and records whether Sedon can preserve forward
...
def _is_foot_geom(name: str) -> bool:
    """Return whether a geom is part of the Sedon foot collision assembly."""

    return name.startswith("R_foot_collision") or name.startswith("L_foot_collision")


def _side_for_geom(name: str) -> str:
    """Return a stable side label for one Sedon foot geom name."""
...
def _world_to_body_local(env: SedonStandingEnv, body_id: int, world_pos: np.ndarray) -> np.ndarray:
...
def _foot_contact_forces(env: SedonStandingEnv) -> dict[str, float | bool | int]:
    """Return side/region contact forces for all Sedon foot geoms.
