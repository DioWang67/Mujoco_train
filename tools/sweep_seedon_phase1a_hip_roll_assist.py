"""Sweep scripted Seedon hip-roll assist for Phase 1A load-transfer reachability."""
...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "seedon" / "march_phase1a_v2_load_transfer_roll_0030.json"
DEFAULT_OUT_CSV = REPO_ROOT / "artifacts" / "seedon_debug" / "phase1a_hip_roll_assist_sweep.csv"
...
def _load_config(path: Path) -> SeedonStandingConfig:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"Seedon config must be a JSON object: {path}")
    return SeedonStandingConfig(**{**SeedonStandingConfig().__dict__, **payload})


def _assist_vector(env: SeedonStandingEnv, amplitude: float) -> np.ndarray:
...
def _install_scripted_assist(env: SeedonStandingEnv, amplitude: float) -> None:
...
def _actuator_ctrl_saturation(env: SeedonStandingEnv) -> float:
    """Return fraction of actuators close to ctrl range limits."""
    return env._torque_saturation_fraction()


def _actuator_force_saturation(env: SeedonStandingEnv) -> float:
