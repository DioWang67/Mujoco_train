...
DEFAULT_SEDON_TUNING = REPO_ROOT / "configs" / "sedon" / "sedon_v5_22_duck_guided_foot_tuning.yaml"
DEFAULT_SEDON_PLAN = REPO_ROOT / "docs" / "sedon_v5_22_duck_guided_foot_tuning_plan.md"
...
def build_payload(xml_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "safety": {
            ...
            "duck_result_directly_applied_to_sedon": False,
...
def sedon_tuning_config(reference_payload: dict[str, Any]) -> dict[str, Any]:
    """Build Sedon v5_22 Duck-guided foot tuning plan config."""
...
def search_axis(reference_field: dict[str, Any], *, delta: float, step: float) -> dict[str, Any]:
    """Build one Sedon tuning search axis."""
...
def write_sedon_plan(path: Path, tuning: dict[str, Any]) -> None:
    """Write Sedon Duck-guided tuning plan Markdown."""
    ...
    if tuning["execution_policy"]["requires_manual_duck_patch_review"]:
        lines.append("- Duck XML does not expose explicit toe/center/heel primitive patches, so manual Duck foot contact review is required before running Duck-guided Sedon tuning.")
    else:
        lines.append("- Use the Duck normalized centers as initial Sedon contact persistence tuning centers, with small bounded sweeps only.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sedon-tuning", type=Path, default=DEFAULT_SEDON_TUNING)
    parser.add_argument("--sedon-plan", type=Path, default=DEFAULT_SEDON_PLAN)
...
def main() -> int:
    ...
    tuning = sedon_tuning_config(payload)
    write_json(args.output, payload)
    write_reference_report(args.report, payload)
    write_json(args.sedon_tuning, tuning)
    write_sedon_plan(args.sedon_plan, tuning)
