"""Generate the Sedon v5_22 MuJoCo scene from the updated URDF package.
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "private_assets" / "sedon_v5_22"
DEFAULT_SCENE_PATH = DEFAULT_OUTPUT_ROOT / "training_scene.xml"
DEFAULT_METADATA_PATH = DEFAULT_OUTPUT_ROOT / "sedon_v5_22_scene_metadata.json"
...
def build_mjcf(
    *,
    ...
    """Build the Sedon v5_22 MJCF tree."""

    mjcf = ET.Element("mujoco", {"model": "sedon_v5_22"})
    ...
    add_assumption_comment(
        actuator,
        "MuJoCo motor stubs copied from prior Sedon simulation convention; not verified motor specs",
    )
...
def add_eval_collision_proxy(body: ET.Element, link_name: str) -> None:
    ...
    if link_name == "base_link":
        add_assumption_comment(body, "base_proxy added for existing Sedon eval contact/fall semantics")
        ET.SubElement(
            body,
            "geom",
            {
...
def add_foot_collision(body: ET.Element, name: str, pos: str) -> None:
    ...
    add_assumption_comment(body, f"{name} box copied from prior Sedon simulation convention")
    ET.SubElement(
        body,
        "geom",
        {
        ...
        "assumptions": [
            {
                ...
                "source": "assumption",
                "confidence": "low",
                "reason": "No verified motor/controller spec found for Sedon v5_22.",
            },
            {
            ...
            },
            {
                ...
                "source": "assumption",
                "confidence": "low",
                "reason": "Existing Sedon env expects R_joint_knee_pitch and R_joint_ankle_pitch.",
            },
            {
                "field": "foot_collision_boxes",
                "value": "prior Sedon MuJoCo convention",
                "source": "assumption",
                "confidence": "low",
...
def main() -> int:
    """Generate the Sedon v5_22 MJCF scene."""

    args = parse_args()
    output_root = args.output_root
    scene_path = output_root / "training_scene.xml"
    metadata_path = output_root / "sedon_v5_22_scene_metadata.json"
