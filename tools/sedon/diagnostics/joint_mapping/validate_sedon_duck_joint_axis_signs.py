"""Generate a conservative Sedon/Duck joint axis and sign readiness report.
...
TOOL_VERSION = "sedon-duck-joint-axis-sign-validation-v1"
UNKNOWN = "unknown"
MANUAL_REQUIRED = "manual_required"

REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_MAPPING = REPO_ROOT / "references" / "open_duck_mini" / "sedon_duck_joint_mapping.yaml"
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "sedon" / "sedon_duck_joint_sign_mapping.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "sedon_duck_joint_axis_sign_report.md"
...
def build_sign_mapping(mapping: dict[str, Any], source_path: Path) -> dict[str, Any]:
    ...
    for item in mapping.get("joint_mappings", []):
        sedon_axis_state = axis_status(item.get("sedon_axis"))
        duck_axis_state = axis_status(item.get("duck_axis"))
        sign_validated = sedon_axis_state == "explicit" and duck_axis_state == "explicit"
        ...
        records.append(
            {
                ...
                "sedon_joint": item.get("sedon_joint"),
                "duck_joint": item.get("duck_joint"),
                "sedon_axis": item.get("sedon_axis"),
                "duck_axis": item.get("duck_axis"),
                "sedon_axis_status": sedon_axis_state,
                "duck_axis_status": duck_axis_state,
                "sedon_range": item.get("sedon_range"),
                "duck_range": item.get("duck_range"),
                "sedon_range_status": range_status(item.get("sedon_range")),
                ...
                "source": {
                    ...
                    "sedon_axis": "semantic mapping extracted from Sedon XML" if sedon_axis_state == "explicit" else UNKNOWN,
    ...
    return {
        ...
        "purpose": "Read-only Sedon/Duck semantic joint axis and sign readiness gate.",
        "sources": {
            "semantic_mapping": str(source_path),
            "sedon_parameters": mapping.get("source_files", {}).get("sedon_parameters"),
