"""Inspect Sedon actuator envelope fields from extracted robot parameters.

This is a read-only diagnostic. It records explicit actuator fields from
`configs/sedon/sedon_robot_parameters.yaml` and marks missing actuator envelope
...
TOOL_VERSION = "sedon-actuator-envelope-inspector-v1"
UNKNOWN = "unknown"
MANUAL_REQUIRED = "manual_required"

REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_SEDON_YAML = REPO_ROOT / "configs" / "sedon" / "sedon_robot_parameters.yaml"
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "sedon" / "sedon_actuator_envelope.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "sedon_actuator_envelope_report.md"
...
def side_from_joint(joint_name: str | None) -> str:
    """Infer side from Sedon joint naming convention."""
...
def build_envelope(robot: dict[str, Any], source_path: Path) -> dict[str, Any]:
        ...
        actuator_records.append(
            {
                ...
                "source": "Explicit actuator fields extracted from Sedon MJCF XML.",
                "notes": "Envelope is not a torque limit unless Sedon control semantics confirm it.",
            }
        )
    ...
    return {
        ...
        "purpose": "Read-only Sedon actuator envelope inspection for Duck-like gait readiness.",
        "sources": {
            "sedon_parameters": str(source_path),
        ...
        "summary": {
            ...
            "reason": "Sedon actuator ctrlrange is explicit, but kp/forcerange/gear and control semantics require clarification.",
        },
        "actuators": actuator_records,
        "manual_required": [
            {
                "field": "sedon_actuator_kp",
                "count": missing_kp,
                "reason": "Sedon XML exposes motor actuators without explicit kp.",
            },
            {
                "field": "sedon_actuator_forcerange",
                "count": missing_forcerange,
                "reason": "Sedon XML exposes no explicit actuator force range.",
            },
            {
                "field": "sedon_actuator_gear",
                "count": missing_gear,
                "reason": "Sedon XML exposes no explicit actuator gear in extracted fields.",
            },
            {
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Sedon Actuator Envelope Report",
        "",
        "Generated by `tools/sedon/diagnostics/actuator/inspect_sedon_actuator_envelope.py`.",
        "",
        "## Summary",
        "",
...
def main() -> int:
    ...
    args = parse_args()
    try:
        robot = load_json_yaml(args.sedon_yaml)
        payload = build_envelope(robot, args.sedon_yaml)
