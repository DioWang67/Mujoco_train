"""Compare Sedon foot-contact prototype variants.

This Class C diagnostic runs short MuJoCo contact checks only. It does not
modify the source Sedon model, train.py, eval.py, or PPO settings, and it does
...
DEFAULT_REPORT = REPO_ROOT / "docs" / "sedon_foot_contact_variant_comparison.md"
...
def side_for_geom(name: str) -> str:
    """Classify Sedon side from a geom name."""
        ...
        rows.append(
            {
                ...
                "source": manifest_item.get("source", "sedon_source_scene"),
                "confidence": manifest_item.get("confidence", "source_scene"),
                "valid_for": manifest_item.get("valid_for", "existing_sedon_contact"),
