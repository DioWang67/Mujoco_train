...
from tools.sedon_blue_like_phase1_rollover_diagnostic import _region_for_geom, _side_for_geom
...
def _left_foot_pos(env: SedonStandingEnv, data: mujoco.MjData | None = None) -> np.ndarray:
...
def _set_joint_positions(env: SedonStandingEnv, data: mujoco.MjData, joint_positions: np.ndarray) -> None:
...
def _kinematic_left_foot_delta(env: SedonStandingEnv, delta: np.ndarray) -> np.ndarray:
...
def _contact_forces(env: SedonStandingEnv) -> ContactForces:
    forces = {
        "left_center": 0.0,
        "left_toe": 0.0,
        "left_heel": 0.0,
...
def _right_support_target(env: SedonStandingEnv, alpha: float, magnitude: float) -> np.ndarray:
...
def _settle_env(env: SedonStandingEnv, seed: int, steps: int = 20) -> None:
    env.reset(seed=seed)
    target = env._nominal_joint_qpos.copy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    command = f".venv\\Scripts\\python.exe -m tools.sedon_phase2c_contact_constrained_foot_mapping --steps {args.steps}"
