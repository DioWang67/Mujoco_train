"""Generate Sedon contact patch readiness data from extracted parameters.

This read-only diagnostic classifies foot-related geoms from the existing
Sedon parameter snapshot. It does not run MuJoCo, move artifacts, or modify
...
TOOL_VERSION = "sedon-contact-patch-readiness-v1"
UNKNOWN = "unknown"
MANUAL_REQUIRED = "manual_required"

REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_SEDON_YAML = REPO_ROOT / "configs" / "sedon" / "sedon_robot_parameters.yaml"
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "sedon" / "sedon_contact_patch_status.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "sedon_contact_patch_readiness_report.md"
...
def classify_geom(geom: dict[str, Any]) -> dict[str, Any]:
    """Classify one Sedon geom for contact readiness."""
    ...
    return {
        ...
        "source": "Sedon extracted geom fields from XML.",
        "reason": reason,
    }
...
def build_contact_status(robot: dict[str, Any], source_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "purpose": "Read-only Sedon foot contact patch readiness for Duck-like gait preparation.",
        "sources": {
        ...
        "manual_required": [
            ...
            },
            {
                ...
                "reason": "Duck foot is mesh based; Sedon box support cannot be assumed equivalent.",
            },
        ],
...
def main() -> int:
    ...
    try:
        ...
        payload = build_contact_status(robot, args.sedon_yaml)
