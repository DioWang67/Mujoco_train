"""Sweep single-support hold candidates for the Seedon MuJoCo model.

This diagnostic does not train PPO and does not change reward/gait logic.

Example:
    python -m tools.debug_seedon_single_support_sweep --relaxed-foot
"""
...
from tools.seedon_debug_common import DEBUG_OUT_DIR, RELAXED_FOOT_SIZE
...
def _apply_relaxed_foot_if_requested(env: SeedonStandingEnv, relaxed_foot: bool) -> None:
...
def _step_with_target(env: SeedonStandingEnv, target_positions: np.ndarray) -> None:
    for _ in range(env.frame_skip):
        env.data.ctrl[:] = env._pd_control(target_positions)
        mujoco.mj_step(env.model, env.data)


def _contact_flags(env: SeedonStandingEnv, lifted_leg: str) -> tuple[bool, bool, int]:
