"""Check Sedon march-in-place Phase 1/2 gates before enabling micro-lift."""
...
PHASE1_CONFIG = REPO_ROOT / "configs" / "sedon" / "march_in_place_phase1_load_transfer.json"
PHASE2_CONFIG = REPO_ROOT / "configs" / "sedon" / "march_in_place_phase2_swing_unload.json"
...
def _load_config(path: Path) -> SedonStandingConfig:
    ...
    Args:
        path: JSON object containing SedonStandingConfig overrides.

    Returns:
        Complete SedonStandingConfig.
    ...
    """
    if not path.is_file():
        raise FileNotFoundError(f"Sedon march config not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"Sedon march config must be a JSON object: {path}")
...
def _evaluate_phase(
    *,
    ...
    terminated = False
    truncated = False
    raw_env: SedonStandingEnv | None = None

    def make_env() -> SedonStandingEnv:
        nonlocal raw_env
        raw_env = SedonStandingEnv(reset_noise_scale=0.0, reward_config=config)
        return raw_env
    ...
    try:
        ...
        for _ in range(steps):
            ...
            if terminated or truncated:
                break
        if raw_env is None:
            raise RuntimeError("Sedon gate checker failed to create an environment.")
        dt = float(raw_env.dt)
    finally:
        vec_env.close()
