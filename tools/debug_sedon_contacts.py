"""Inspect Sedon training-scene proxy geoms and initial contacts."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to Sedon training_scene.xml.",
    )
    parser.add_argument(
        "--raw-scene-reset",
        action="store_true",
        help="Inspect MuJoCo's raw XML reset instead of SedonStandingEnv.reset().",
    )
    return parser


def main(argv: list[str] | None = None) -> int:
    """Print Sedon contact proxy diagnostics for the reset pose."""
    args = build_parser().parse_args(argv)
    scene_path = require_scene(args.scene)
    env: SedonStandingEnv | None = None
    if args.raw_scene_reset:
        ...
        env = SedonStandingEnv(scene_path=scene_path, reset_noise_scale=0.0)
        env.reset(seed=42)
        model = env.model
        data = env.data
        reset_mode = "SedonStandingEnv.reset(seed=42)"
