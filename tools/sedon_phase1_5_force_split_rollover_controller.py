"""Phase 1.5 force-split and rollover controller for Sedon.
...
def _target(env: SedonStandingEnv, runtime: Phase15Runtime, config: Phase15Config) -> tuple[np.ndarray, float, float, float]:
...
def _append_progress_log(command: str, summary: Phase15Summary, output_dir: Path) -> None:
    ...
    if not PROGRESS_LOG.exists():
        PROGRESS_LOG.write_text(
            ...
            "This append-only log tracks Sedon Blue / BDX-like dynamic gait experiments, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    command = (
        ".venv\\Scripts\\python.exe -m tools.sedon_phase1_5_force_split_rollover_controller "
