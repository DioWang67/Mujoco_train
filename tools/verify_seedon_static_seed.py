"""Verify that a Seedon standing seed remains safe under zero action."""
...
DEFAULT_CONFIG_PATH = (
    Path(__file__).resolve().parents[1] / "configs" / "seedon" / "zero_action_safe_stand.json"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to a Seedon config JSON file.",
    )
...
def load_reward_config(path: Path) -> SeedonStandingConfig:
    """Load a ``SeedonStandingConfig`` from a JSON file."""
    if not path.is_file():
        raise FileNotFoundError(f"Config file not found: {path}")
    raw_payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(raw_payload, dict):
        raise ValueError(f"Config file must contain a JSON object: {path}")
    return SeedonStandingConfig(**raw_payload)
...
def run_zero_action_rollout(
    ...
    *,
    ...
    """Run a zero-action Seedon rollout and aggregate safety metrics."""
    if steps <= 0:
    ...
    try:
        ...
        for step_index in range(steps):
            ...
            if truncated:
                raise RuntimeError("Seedon static-seed verifier received unexpected truncation.")
