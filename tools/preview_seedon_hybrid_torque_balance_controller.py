"""
Preview Seedon hybrid torque balance controller.
...
Usage:
    .\\.venv\\Scripts\\python.exe -m tools.preview_seedon_hybrid_torque_balance_controller --steps 320 --print-every 20

Conservative test:
    .\\.venv\\Scripts\\python.exe -m tools.preview_seedon_hybrid_torque_balance_controller --steps 320 --print-every 20 --target-force-ratio 0.58 --max-tau-assist 2.0 --kp-force-tau 4.0
"""
...
def build_base_target_positions(env: SeedonStandingEnv) -> np.ndarray:
    if not hasattr(env, "_nominal_joint_qpos"):
        raise RuntimeError("SeedonStandingEnv must expose _nominal_joint_qpos for this preview tool.")
...
def apply_safe_clamps(env: SeedonStandingEnv, target_positions: np.ndarray) -> np.ndarray:
...
def build_command(env: SeedonStandingEnv, state: RobotState, memory: ControllerMemory, cfg: ControllerConfig) -> StepCommand:
    phase = memory.phase
...
def compute_pd_ctrl(env: SeedonStandingEnv, target_positions: np.ndarray, cfg: ControllerConfig) -> np.ndarray:
    model, data = _safe_get_model_data(env)
...
def inject_hip_roll_torque(env: SeedonStandingEnv, ctrl: np.ndarray, left_tau: float, right_tau: float) -> np.ndarray:
...
def hybrid_step_env(env: SeedonStandingEnv, command: StepCommand, cfg: ControllerConfig) -> None:
    """
    Step using SeedonStandingEnv's existing PD simulation path.
    ...
    if not hasattr(env, "_do_pd_simulation"):
        raise RuntimeError("SeedonStandingEnv must expose _do_pd_simulation for this preview tool.")
...
def build_log_row(state: RobotState, command: StepCommand, memory: ControllerMemory, env: SeedonStandingEnv) -> Dict[str, object]:
    model, data = _safe_get_model_data(env)
    j = JointNames()
...
def run_controller(args: argparse.Namespace) -> Dict[str, object]:
    ...
    env = SeedonStandingEnv(scene_path=DEFAULT_SCENE_PATH.resolve())
    reset_env(env)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Preview Seedon hybrid torque balance controller.")
