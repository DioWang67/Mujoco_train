"""Phase G1 raw contact-pair diagnostic for Seedon foot geometry.
...
force, and no toe handoff. It does not train, edit rewards, modify Seedon
...
from tools.seedon_debug_common import DEBUG_OUT_DIR, DEFAULT_SCENE_PATH, geom_name, geom_type_name, require_scene
...
def side_for_geom(name: str) -> str:
    """Classify a Seedon foot geom side from its name."""
    lowered = name.lower()
...
def region_for_geom(name: str) -> str:
    """Classify a Seedon foot geom contact region from its name."""
    lowered = name.lower()
    if "toe" in lowered:
        return "toe"
    if "heel" in lowered:
