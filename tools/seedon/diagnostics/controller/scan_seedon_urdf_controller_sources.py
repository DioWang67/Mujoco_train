"""Scan Seedon URDF and controller source candidates.
...
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "seedon" / "seedon_control_source_candidates.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "seedon_urdf_controller_source_scan.md"
SCAN_ROOTS = (
    "private_assets",
    "configs",
    "seedon_baseline",
...
def python_control_priority(finding: dict[str, Any]) -> tuple[int, str, int]:
    ...
    field = finding["field"]
    if source == "seedon_baseline/env.py":
        source_priority = 0
    elif source.startswith("private_assets/seedon/"):
        source_priority = 1
    elif source.startswith("seedon_baseline/"):
        source_priority = 2
    elif source.startswith("configs/seedon/"):
        source_priority = 3
    else:
        source_priority = 4
...
def scan_python_control(files: list[Path]) -> list[dict[str, Any]]:
    ...
    for path in files:
        ...
        if not (
            source.startswith("seedon_baseline/")
            or source.startswith("tools/")
            or source.startswith("configs/seedon/")
            or source.startswith("private_assets/seedon/")
        ):
            continue
...
def scan_motor_specs(files: list[Path]) -> list[dict[str, Any]]:
    ...
    for path in files:
        source = rel_path(path)
        if not (
            source.startswith("private_assets/seedon/")
            or source.startswith("configs/seedon/")
            or source.startswith("seedon_baseline/")
        ):
            continue
