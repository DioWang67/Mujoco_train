"""Estimate conservative Seedon/Open Duck Mini scale reference fields.

This tool reads existing extracted parameter snapshots and writes a read-only
scale mapping table. It does not apply Duck parameters to Seedon and does not
...
TOOL_VERSION = "seedon-duck-scale-fields-v1"
UNKNOWN = "unknown"
...
DEFAULT_OUTPUT_YAML = REPO_ROOT / "configs" / "seedon" / "seedon_duck_scale_mapping.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "seedon_duck_scale_table.md"
...
def ratio(seedon_value: float | int | None, duck_value: float | int | None) -> float | None:
    """Return Seedon/Duck ratio when both values are numeric.

    Args:
        seedon_value: Seedon scalar.
    ...
    if not isinstance(seedon_value, (int, float)) or not isinstance(duck_value, (int, float)):
        return None
    if duck_value == 0:
        return None
    return round(float(seedon_value) / float(duck_value), 6)
...
def scalar_field(
    ...
    return {
        "field": name,
        "unit": unit,
        "seedon_value": seedon_value,
        "duck_value": duck_value,
        "seedon_to_duck_ratio": ratio(seedon_value, duck_value),
        "source": source,
...
def build_scale_mapping(seedon: dict[str, Any], duck: dict[str, Any], seedon_path: Path, duck_path: Path) -> dict[str, Any]:
    ...
    seedon_chain_lengths = {
        "left": chain_path_length(seedon, seedon_left_chain),
        "right": chain_path_length(seedon, seedon_right_chain),
    }
    duck_chain_lengths = {
        "left": chain_path_length(duck, duck_left_chain),
        "right": chain_path_length(duck, duck_right_chain),
    }
    seedon_avg_chain = average_numeric(list(seedon_chain_lengths.values()))
    ...
    scale_fields = [
        ...
        scalar_field(
            "hip_lateral_spacing",
            lateral_spacing(seedon, "L_link_hip_yaw", "R_link_hip_yaw"),
        ...
        scalar_field(
            ...
            seedon_counts.get("foot_related_geoms"),
    ...
    seedon_foot_box = foot_box_full_extents(seedon)
    duck_foot_box = foot_box_full_extents(duck)
    seedon_foot_bbox = foot_geom_position_bbox(seedon)
    ...
    return {
        ...
        "purpose": "Read-only normalized scale references for Seedon/Duck gait mapping preparation.",
        "sources": {
            "seedon_parameters": str(seedon_path),
        ...
        "summary": {
            ...
            "primary_ratio": {
                ...
                "seedon_to_duck_ratio": ratio(seedon_mass, duck_mass),
                "confidence": "high",
            },
            "usable_for": [
        ...
        "shape_fields": {
            "seedon_foot_box_full_extents_m": seedon_foot_box,
            "duck_foot_box_full_extents_m": duck_foot_box,
            "seedon_foot_geom_local_position_bbox_m": seedon_foot_bbox,
            "duck_foot_geom_local_position_bbox_m": duck_foot_bbox,
            "local_leg_chain_path_lengths_m": {
                "seedon": seedon_chain_lengths,
        ...
        "safety_notes": [
            ...
            "Do not apply Duck kp, forcerange, or joint trajectories to Seedon.",
...
def write_report(path: Path, mapping: dict[str, Any]) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Seedon Duck Normalized Scale Table",
        "",
        "Generated by `tools/seedon/extractors/estimate_seedon_duck_scale_fields.py`.",
        "",
        "## Summary",
        "",
        "- Status: `REFERENCE_ONLY`",
        "- Direct Duck parameter transfer allowed: `false`",
        f"- Primary explicit mass ratio Seedon/Duck: `{mapping['summary']['primary_ratio']['seedon_to_duck_ratio']}`",
        "- This table is for normalization planning and manual diagnostics only.",
        "",
        "## Sources",
        "",
        f"- Seedon parameters: `{mapping['sources']['seedon_parameters']}`",
        f"- Duck parameters: `{mapping['sources']['duck_parameters']}`",
        f"- Generated at: `{mapping['generated_at']}`",
        "",
        "## Scale Fields",
        "",
        "| Field | Seedon | Duck | Seedon/Duck | Unit | Confidence | Source | Notes |",
        "|---|---:|---:|---:|---|---|---|---|",
    ]
    for field in mapping["scale_fields"]:
        lines.append(
            ...
            + " | ".join(
                [
                    f"`{field['field']}`",
                    format_value(field["seedon_value"]),
                    format_value(field["duck_value"]),
                    format_value(field["seedon_to_duck_ratio"]),
                    f"`{field['unit']}`",
                    f"`{field['confidence']}`",
                    field["source"],
                    field["notes"],
    ...
    lines.extend(
        [
            ...
            "|---|---|---|---|",
            f"| `seedon_foot_box_full_extents_m` | {format_value(shape['seedon_foot_box_full_extents_m']['value'])} | `{shape['seedon_foot_box_full_extents_m']['confidence']}` | {shape['seedon_foot_box_full_extents_m']['source']} |",
            ...
            f"| `seedon_foot_geom_local_position_bbox_m` | {format_value(shape['seedon_foot_geom_local_position_bbox_m']['value'])} | `{shape['seedon_foot_geom_local_position_bbox_m']['confidence']}` | {shape['seedon_foot_geom_local_position_bbox_m']['source']} |",
...
def main() -> int:
    ...
    args = parse_args()
    try:
        seedon = load_json_yaml(args.seedon_yaml)
        duck = load_json_yaml(args.duck_yaml)
        mapping = build_scale_mapping(seedon, duck, args.seedon_yaml, args.duck_yaml)
