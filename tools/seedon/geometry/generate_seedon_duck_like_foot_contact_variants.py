"""Generate Seedon foot-contact prototype scene variants.
...
from tools.seedon_debug_common import DEBUG_OUT_DIR, DEFAULT_SCENE_PATH, require_scene


REPO_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_PROFILE_PATH = REPO_ROOT / "configs" / "seedon" / "seedon_foot_contact_prototype_profiles.yaml"
DEFAULT_OUT_DIR = DEBUG_OUT_DIR / "foot_contact_variants"
DEFAULT_PLAN_DOC = REPO_ROOT / "docs" / "seedon_foot_contact_prototype_plan.md"
PROTOTYPE_METADATA = {
...
class FootGeom:
    ...
    Args:
        ...
        geom_name: Original Seedon foot collision geom name.
...
def resolve_source_feet(root: ET.Element) -> dict[str, FootGeom]:
    """Resolve current Seedon foot collision boxes from the source scene."""
...
def foot_length_status(feet: dict[str, FootGeom]) -> tuple[bool, str]:
    ...
    return True, "resolved_from_seedon_foot_collision_box_size"
...
def write_variant_scene(
    ...
    root.set("model", f"seedon_{profile_name}")
...
def write_plan_doc(path: Path, manifest: dict[str, Any]) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Seedon Foot Contact Prototype Plan",
        "",
        "Task class: Class C experiment workflow. This plan creates artifact-only contact prototypes for MuJoCo diagnostics.",
        "",
        "## Safety Constraints",
        "",
        "- Does not modify `private_assets/seedon/training_scene.xml`.",
        "- Does not modify `seedon_baseline/train.py` or `seedon_baseline/eval.py`.",
        "- Does not delete or move artifacts.",
        "- Does not enter PPO and does not claim walking success.",
        "- Open Duck Mini foot geometry is concept/reference only, not Seedon verified geometry.",
    ...
    lines.extend(
        [
            ...
            "## Trade-off",
            "",
            "The prototype disables original foot collision only inside generated variant XMLs so patch-level contacts can be classified. This improves diagnostic clarity, but it means the variants are not physical Seedon geometry and must stay simulation-prototype-only.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    """Generate artifact-only Seedon foot contact prototype variants."""
