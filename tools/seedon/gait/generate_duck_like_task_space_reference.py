"""Generate a gated Duck-like task-space reference skeleton for Seedon.
...
TOOL_VERSION = "seedon-duck-like-task-space-reference-v1"
UNKNOWN = "unknown"
MANUAL_REQUIRED = "manual_required"

REPO_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_SCALE = REPO_ROOT / "configs" / "seedon" / "seedon_duck_scale_mapping.yaml"
DEFAULT_SIGN = REPO_ROOT / "configs" / "seedon" / "seedon_duck_joint_sign_mapping.yaml"
DEFAULT_CONTACT = REPO_ROOT / "configs" / "seedon" / "seedon_contact_patch_status.yaml"
DEFAULT_DUCK_GAIT = REPO_ROOT / "references" / "open_duck_mini" / "duck_gait_reference_metadata.yaml"
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "seedon" / "seedon_duck_like_gait_reference.yaml"
DEFAULT_REPORT = REPO_ROOT / "docs" / "seedon_duck_like_task_space_reference.md"
...
def scale_ratio(scale: dict[str, Any], field_name: str) -> Any:
    ...
    for field in scale.get("scale_fields", []):
        if field.get("field") == field_name:
            return field.get("seedon_to_duck_ratio")
    return None
...
def build_reference(scale: dict[str, Any], sign: dict[str, Any], contact: dict[str, Any], duck_gait: dict[str, Any]) -> dict[str, Any]:
    ...
    if not contact_validated:
        blockers.append("seedon_contact_simulation_validation_missing")
    ...
    return {
        ...
        "purpose": "Duck-like task-space gait reference skeleton for Seedon readiness workflow.",
        "sources": {
        ...
        "normalization_references": {
            "explicit_body_mass_ratio_seedon_to_duck": {
                "value": scale_ratio(scale, "explicit_body_mass"),
                "source": "configs/seedon/seedon_duck_scale_mapping.yaml",
                "confidence": "high",
            },
            "base_height_ratio_seedon_to_duck": {
                "value": scale_ratio(scale, "initial_base_height_z"),
                "source": "configs/seedon/seedon_duck_scale_mapping.yaml",
                "confidence": "medium",
            },
            "hip_lateral_spacing_ratio_seedon_to_duck": {
                "value": scale_ratio(scale, "hip_lateral_spacing"),
                "source": "configs/seedon/seedon_duck_scale_mapping.yaml",
                "confidence": "medium",
            },
            "local_leg_chain_path_ratio_seedon_to_duck": {
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Seedon Duck-Like Task-Space Reference",
        "",
        "Generated by `tools/seedon/gait/generate_duck_like_task_space_reference.py`.",
        "",
        "## Summary",
        "",
