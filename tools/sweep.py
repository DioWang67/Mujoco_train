...
Usage:
    python -m tools.sweep                    # run 20 trials
    python -m tools.sweep --n-trials 50      # run 50 trials
    python -m tools.sweep --steps 1000000    # longer per-trial budget
"""
...
def objective(trial: optuna.Trial, total_steps: int) -> float:
    ...
    trial_dir = SWEEP_DIR / f"trial_{trial.number:04d}"
