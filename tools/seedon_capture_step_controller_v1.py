"""Simplified capture-step controller skeleton for Seedon.
...
def _side_index(side: str) -> int:
    """Return Seedon foot geom index for a side."""
...
def _capture_target_xy(env: SeedonStandingEnv, runtime: CaptureRuntime, config: CaptureConfig) -> tuple[float, float]:
...
def _swing_foot_xy(env: SeedonStandingEnv, runtime: CaptureRuntime) -> tuple[float, float, float]:
...
def _toe_center_contact_for_side(env: SeedonStandingEnv, side: str) -> tuple[bool, bool]:
