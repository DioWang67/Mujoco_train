"""Explicit Sedon locomotion controller v2 skeleton and audit."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "artifacts" / "sedon_debug" / "controller_v2"
...
def _preload_target(env: SedonStandingEnv, swing_side: str, config: ControllerConfig) -> np.ndarray:
...
def _set_joint_positions_in_data(env: SedonStandingEnv, data: mujoco.MjData, joint_positions: np.ndarray) -> None:
...
def _kinematic_foot_z(env: SedonStandingEnv, joint_positions: np.ndarray, side: str) -> float:
...
def _ik_delta_for_foot_z(env: SedonStandingEnv, swing_side: str, target_z: float, config: ControllerConfig) -> np.ndarray:
...
def _enter_phase(runtime: ControllerRuntime, phase: LocomotionPhase, env: SedonStandingEnv) -> None:
    runtime.phase = phase
...
def _controller_target(env: SedonStandingEnv, runtime: ControllerRuntime, config: ControllerConfig) -> tuple[np.ndarray, float, str]:
...
def run_controller(
    ...
    *,
    ...
    env = SedonStandingEnv(reset_noise_scale=0.0, reward_config=env_config)
    runtime = ControllerRuntime()
...
def write_summary(path: Path, rows: list[ControllerAudit], teacher_impact: float) -> None:
    lines = [
        "# Sedon Explicit Locomotion Controller V2",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_results(args.out_dir / "sedon_explicit_locomotion_controller_v2.csv", rows)
