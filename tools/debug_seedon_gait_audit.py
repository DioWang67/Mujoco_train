"""Audit Seedon gait rollouts for contact-safe reverse-knee shaping.
...
from seedon_baseline.env import load_seedon_config_from_env
...
class AuditStep:
    """Per-step Seedon gait audit metrics written to CSV."""
...
def _standing_config(config: SeedonStandingConfig) -> SeedonStandingConfig:
...
def _base_roll_pitch(env: SeedonStandingEnv) -> tuple[float, float]:
...
def _foot_floor_load(env: SeedonStandingEnv, side: str) -> tuple[bool, float]:
...
def _contact_flags(env: SeedonStandingEnv) -> tuple[bool, bool, bool, bool]:
...
def _policy_action_provider(model_path: Path, vecnorm_path: Path | None, env: SeedonStandingEnv):
...
def _diagnose(rows: list[AuditStep], terminated: bool) -> str:
    """Classify a Seedon rollout using conservative gait-shaping gates."""
    if not rows:
...
def run_audit(
    *,
    ...
    """Run a Seedon gait audit and write per-step metrics to CSV."""
    if steps <= 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Audit Seedon rollout contact, unload, micro-lift, and knee phase metrics."
    )
    parser.add_argument(
        "--scene-path",
        type=Path,
        default=DEFAULT_SCENE_PATH,
        help="Seedon MJCF scene path.",
    )
    parser.add_argument(
        "--mode",
