"""Minimal validation for Sedon Blue-like rounded sole variants.
...
def _is_foot_geom(name: str) -> bool:
    """Return whether a geom belongs to a Sedon foot collision assembly."""
    return name.startswith("R_foot_collision") or name.startswith("L_foot_collision")


def _side_for_geom(name: str) -> str:
    """Return left/right label for a Sedon foot geom name."""
...
def _foot_contacts(env: SedonStandingEnv) -> list[dict[str, Any]]:
...
def _base_pitch(env: SedonStandingEnv) -> float:
