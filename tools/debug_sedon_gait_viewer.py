"""Visual Sedon gait rollout player with MuJoCo viewer and step diagnostics.

This tool plays zero/scripted/policy Sedon rollouts while printing per-step
...
DEFAULT_OUT_CSV = DEBUG_OUT_DIR / "sedon_gait_viewer.csv"
...
@dataclass(frozen=True)
class GaitSeed:
    """Validated gait seed exported by ``debug_sedon_pose_editor``."""
...
def _load_gait_seed(path: Path) -> GaitSeed:
    ...
    Args:
        path: JSON file produced by ``debug_sedon_pose_editor``.
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            "`python -m tools.debug_sedon_pose_editor`, add keyframes, then click Export sequence."
        )
    ...
    if list(joint_names or []) != list(JOINT_NAMES):
        raise ValueError(
            "Gait seed joint_names do not match Sedon JOINT_NAMES. "
...
def _contact_flags(env: SedonStandingEnv) -> tuple[bool, bool, bool, bool, list[str]]:
    left = False
    right = False
    base = False
    foot_foot = False
...
def _foot_normal_force(env: SedonStandingEnv, foot_geom_name: str) -> float:
    total_force = 0.0
...
def _update_viewer_markers(viewer: object, env: SedonStandingEnv, row: GaitViewerStep) -> None:
...
def _step_seed_reference(
    ...
    """Advance Sedon by one env step while tracking exported reference targets."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    env = SedonStandingEnv(scene_path=scene_path, reset_noise_scale=0.0, reward_config=reward_config)
    try:
