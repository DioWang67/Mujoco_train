"""Diagnose Seedon leg bend direction, foot collision placement, and safe knee ranges."""
...
from tools.seedon_debug_common import DEBUG_OUT_DIR, LEFT_FOOT_GEOM, RIGHT_FOOT_GEOM, snapshot_geom
...
def _body_position(env: SeedonStandingEnv, body_name: str) -> np.ndarray:
...
def main(argv: list[str] | None = None) -> int:
    """Run the Seedon leg-direction diagnostic."""
    args = build_parser().parse_args(argv)
    env = SeedonStandingEnv(reset_noise_scale=0.0, reward_config=SeedonStandingConfig(gait_mode="fsm"))
    try:
        env.reset(seed=42)
