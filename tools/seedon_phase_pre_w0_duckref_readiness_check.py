"""Phase Pre-W0 DuckRef readiness check for Seedon.
...
DEBUG_OUT_DIR = REPO_ROOT / "artifacts" / "seedon_debug"
...
def build_summary(config: ReadinessConfig) -> dict[str, Any]:
    ...
    gait_reference = read_json(config.m0_dir / "seedon_duck_scaled_gait_reference.json")
    m0_sign_mapping = read_json(config.m0_dir / "seedon_joint_sign_mapping.json")
    g1_summary = read_json(config.g1_dir / "phase_g1_summary.json")
    actuator_summary = read_json(config.actuator_dir / "seedon_actuator_semantics_summary.json")
    free_space_summary = read_json(config.free_space_dir / "seedon_free_space_joint_sign_mapping.json")
    ...
    scaled_duckref_too_aggressive_risk = (
        float(gait_reference.get("seedon_target_vx", 1.0)) > 0.10
        or float(gait_reference.get("seedon_foot_clearance_target", 1.0)) > 0.025
        or float(gait_reference.get("seedon_action_scale_initial", 1.0)) >= 0.25
    )
    ...
    return {
        ...
        "source_files": {
            "m0_gait_reference": str(config.m0_dir / "seedon_duck_scaled_gait_reference.json"),
            "m0_sign_mapping": str(config.m0_dir / "seedon_joint_sign_mapping.json"),
            "g1_summary": str(config.g1_dir / "phase_g1_summary.json"),
            "actuator_summary": str(config.actuator_dir / "seedon_actuator_semantics_summary.json"),
            "free_space_summary": str(config.free_space_dir / "seedon_free_space_joint_sign_mapping.json"),
        },
    }
