"""Break down Seedon hip-roll generalized forces under floor contact.

This diagnostic keeps the normal committed Seedon model and reward config. It
...
def _ctrl_saturated(env: SeedonStandingEnv, actuator_id: int, tolerance: float = 1e-6) -> bool:
...
def _hip_roll_target(env: SeedonStandingEnv, support_side: str, hip_roll_offset: float) -> np.ndarray:
...
def _constraint_breakdown_by_dof(env: SeedonStandingEnv) -> tuple[np.ndarray, np.ndarray]:
...
def _dof_breakdown(env: SeedonStandingEnv, dof_index: int) -> DofForceBreakdown:
