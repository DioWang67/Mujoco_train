"""Preview deterministic Seedon gait targets without PPO training or loading."""
...
DEFAULT_OUT_CSV = DEBUG_OUT_DIR / "preview_seedon_gait.csv"
...
def _blue_step_phase(env: SeedonStandingEnv) -> tuple[str, float]:
...
def _blue_step_joint_offsets(env: SeedonStandingEnv) -> tuple[np.ndarray, str]:
...
def _target_positions(env: SeedonStandingEnv, gait_mode: str) -> tuple[np.ndarray, str]:
...
def _pin_base(env: SeedonStandingEnv, base_qpos: np.ndarray) -> None:
...
def _set_floor_contact_enabled(env: SeedonStandingEnv, enabled: bool) -> tuple[int, int]:
...
def _restore_floor_contact(env: SeedonStandingEnv, previous: tuple[int, int]) -> None:
