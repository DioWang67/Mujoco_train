"""Map Seedon foot bottom height over hip/knee/ankle qpos without dynamics."""
...
from seedon_baseline.env import JOINT_NAMES, SeedonStandingConfig, SeedonStandingEnv
from tools.seedon_debug_common import DEBUG_OUT_DIR
...
def _joint_range(env: SeedonStandingEnv, joint_index: int) -> tuple[float, float]:
    """Return the MuJoCo range for a Seedon actuated joint index."""
...
def _in_joint_range(env: SeedonStandingEnv, joint_index: int, value: float) -> bool:
    """Return whether a qpos value is inside the joint range."""
    lower, upper = _joint_range(env, joint_index)
    return lower <= value <= upper


def _set_joint_position(env: SeedonStandingEnv, joint_index: int, value: float) -> None:
    """Set one actuated joint qpos by Seedon joint index."""
    joint_id = env._joint_ids[joint_index]
...
def _nominal_foot_bottoms(env: SeedonStandingEnv) -> np.ndarray:
