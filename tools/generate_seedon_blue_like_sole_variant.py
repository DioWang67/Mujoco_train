"""Generate conservative Blue-like rounded sole scene variants for Seedon.

This Class C experiment helper does not modify the canonical Seedon scene,
...
GAP_REVIEW_PATH = DEBUG_OUT_DIR / "blue_like_seedon_gap_review.md"
...
def write_gap_review(payloads: list[dict[str, Any]], report_path: Path) -> None:
    """Write the Blue/BDX vs Seedon gap review."""
    lines = [
        "# Blue-Like Seedon Gap Review",
        "",
    ...
    lines.extend(
        [
            ...
            "## Current Seedon Gaps",
            "",
            "- Foot geometry: canonical Seedon uses a single flat box foot, so passive roll is not physically represented.",
            ...
            "- Actuation: Seedon has no active ankle roll, so rounded sole/passive roll must carry that role; hip roll range alone was not enough for static load transfer.",
            ...
            "",
            "The new variants are closer to Blue/BDX physical assumptions than both flat-box and full-ellipsoid feet because they keep a stable central patch while adding toe/heel sagittal rocker and mild lateral shoulders. They are still only a contact-shape precondition, not evidence that Seedon can already perform controlled falling or foot catch.",
            "",
            "## Next Metrics",
            "",
