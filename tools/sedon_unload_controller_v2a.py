"""Closed-loop unload-only controller audit for Sedon controller v2a."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "artifacts" / "sedon_debug" / "controller_v2a_unload"
...
def _target(env: SedonStandingEnv, runtime: UnloadRuntime, config: UnloadConfig) -> tuple[np.ndarray, float]:
...
def _advance(runtime: UnloadRuntime, env: SedonStandingEnv, config: UnloadConfig) -> str:
    event = ""
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_results(args.out_dir / "sedon_unload_controller_v2a.csv", rows)
