"""Sweep temporary Seedon foot proxy redesigns for lateral load-transfer diagnostics.
...
def _contact_state(env: SeedonStandingEnv, contact_geom_names: dict[str, tuple[str, ...]]) -> tuple[str, bool]:
...
def _total_proxy_contact_count(env: SeedonStandingEnv, contact_geom_names: dict[str, tuple[str, ...]]) -> int:
...
def _set_meshdir_absolute(root: ET.Element) -> None:
    ...
    if compiler is None:
        raise ValueError("Expected <compiler> element in Seedon training scene.")
...
def _make_case_definition(proxy_type: str, temp_dir: Path) -> ProxyCaseDefinition:
    ...
    output_path = temp_dir / f"seedon_training_scene_{proxy_type}.xml"
...
def main(argv: list[str] | None = None) -> int:
    """Run the Seedon foot-proxy redesign sweep."""
    ...
    with tempfile.TemporaryDirectory(prefix="seedon_foot_proxy_redesign_") as tmp_dir_str:
