"""Shared helpers for Seedon MuJoCo contact/debug tools."""
...
def require_scene(path: Path) -> Path:
    ...
    if not scene_path.is_file():
        raise FileNotFoundError(f"Seedon training scene not found: {scene_path}")
    return scene_path
...
def estimate_geom_bottom_z(model: mujoco.MjModel, data: mujoco.MjData, geom: int) -> float:
    """Estimate the lowest world z for simple proxy geoms used in Seedon."""
...
def apply_foot_size_override(
    ...
    """Apply a temporary foot collision box half-size to both Seedon feet."""
    if len(foot_size) != 3:
