...
def main() -> None:
    ...
    for f in ["train.py", "h1_env.py", "eval.py",
              "plot_eval.py", "sweep.py", "requirements.txt"]:
        src = HERE / f
        if src.exists():
            shutil.copy(src, CODE / f)
    ...
    # ── Pack ─────────────────────────────────────────────────────────────
    archive = "h1_package.tar.gz"
    print(f"\nPacking into {archive} ...")
    rc = run(["tar", "-czf", archive, str(PKG_DIR)])
    if rc != 0:
    ...
    print()
    print("  Transfer:")
    print("    scp h1_package.tar.gz user@remote:~")
    print()
    print("  On remote:")
