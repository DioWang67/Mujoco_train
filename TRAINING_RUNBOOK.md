...
python gate_check.py --report aggregate_compare.json --gates gate_profiles.json --profile preprod --mode aggregate
```
...
`compare_eval.py` 的重點不是看 DR 絕對值，而是看：
...
現在已經修掉，`eval.py` / `compare_eval.py` / `benchmark_matrix.py` 都套了 `TimeLimit(1000)`。
...
3. 到你覺得像成形時，再跑 `eval.py` / `compare_eval.py`
