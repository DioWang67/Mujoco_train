...
- Seedon standing baseline from a private URDF/MJCF conversion flow
...
seedon_baseline/   Seedon standing environment, training, and tests
...
- `docs/TRAINING_RUNBOOK.md`
  Short operator commands for H1, grasp, Seedon, eval, and remote runs.
- `docs/SEEDON_WORKFLOW.md`
  Current Seedon reverse-knee/no-tiptoe workflow, remote command, viewer/audit
  checks, and troubleshooting.

For Seedon config selection, use `configs/seedon/README.md`.
...
Prepare and train Seedon locally:
...
Current Seedon reverse-knee/no-tiptoe experiment:

```powershell
$env:SEEDON_CONFIG_OVERRIDES='configs\seedon\reverse_knee_no_tiptoe_walk.json'
python train.py --project seedon --total-timesteps 10000000 --n-envs 4 --reset-noise-scale 0.01
```

Evaluate Seedon:

```bash
python eval.py --project seedon --episodes 1 --render
python eval.py --project seedon --episodes 1 --record
scripts\seedon_eval.bat
```
...
Seedon has a one-command wrapper:
...
paths. Example: `deploy_content/configs/seedon/foo.json` deploys as
`configs/seedon/foo.json`.
