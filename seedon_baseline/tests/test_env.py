"""Regression tests for the Seedon standing environment."""
...
seedon_env_module = sys.modules[SeedonStandingEnv.__module__]
...
def test_seedon_gait_offsets_keep_configured_joint_signs() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
        gait_mode="sin",
...
def test_seedon_nominal_joint_pose_offsets_apply_symmetrically() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
...
def test_seedon_knee_safe_ranges_return_none_when_unset() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig()
...
def test_seedon_knee_safe_range_violation_reports_excess_distance() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
...
def test_seedon_safe_joint_target_clamps_limit_knees() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
...
def test_reference_march_offsets_track_scaled_absolute_seed() -> None:
    ...
    seed = load_reference_gait_seed(
        Path("configs/seedon/reference_march_pose_1_4_mirrored_seed.json")
    )
...
def test_seedon_termination_reason_reports_specific_failure() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig()
...
def test_seedon_fsm_offsets_honor_configured_double_support_window() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
        gait_mode="fsm",
...
def test_seedon_fsm_walk_phase_metadata_tracks_support_side() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
        task_mode="walk",
        gait_mode="fsm",
...
def test_seedon_fsm_offsets_use_configured_support_roll_scale() -> None:
    env = SeedonStandingEnv.__new__(SeedonStandingEnv)
    env._reward_config = SeedonStandingConfig(
        gait_mode="fsm",
...
def test_do_pd_simulation_with_torque_assist_clamps_and_records_actual_delta(
    ...
    monkeypatch.setattr(seedon_env_module, "mujoco", SimpleNamespace(mj_step=fake_mj_step))
...
@pytest.fixture
def seedon_env_class() -> type[SeedonStandingEnv]:
    """Return the Seedon env class, skipping when private assets are unavailable."""
    pytest.importorskip("mujoco")
    if not Path(DEFAULT_SCENE_PATH).is_file():
        pytest.skip("Seedon private training_scene.xml has not been generated.")
    return SeedonStandingEnv


def test_seedon_env_reset_returns_expected_observation_shape(
    seedon_env_class: type[SeedonStandingEnv],
) -> None:
    env = seedon_env_class(reset_noise_scale=0.0)
    try:
...
def test_seedon_env_zero_action_step_stays_numeric(
    seedon_env_class: type[SeedonStandingEnv],
) -> None:
    env = seedon_env_class(reset_noise_scale=0.0)
    try:
