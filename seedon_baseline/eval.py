"""Evaluate a trained Seedon standing policy.

Usage:
    python -m seedon_baseline.eval --episodes 5
    python -m seedon_baseline.eval --model-path models/seedon/latest_model.zip
    python -m seedon_baseline.eval --episodes 1 --render
    python -m seedon_baseline.eval --episodes 1 --record
"""
...
from seedon_baseline.train import MAX_EPISODE_STEPS

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_MODELS_ROOT = REPO_ROOT / "models" / "seedon"
DEFAULT_TRAIN_CONFIG_PATH = REPO_ROOT / "logs" / "seedon" / "train_config.json"
DEFAULT_REPORT_PATH = REPO_ROOT / "reports" / "seedon_eval.csv"
DEFAULT_VIDEO_PATH = REPO_ROOT / "reports" / "seedon_eval.gif"
...
class SeedonEvalEpisode:
    """Metrics collected from one Seedon evaluation episode."""
...
def resolve_model_path(models_root: Path, explicit_model_path: Path | None) -> Path:
    """Resolve the Seedon policy checkpoint to evaluate.

    Args:
        models_root: Directory containing Seedon checkpoints.
    ...
    candidates.extend(
        sorted(
            models_root.glob("seedon_ppo_*_steps.zip"),
    ...
    raise FileNotFoundError(
        "No readable Seedon checkpoint found. Expected a valid best/best_model.zip, "
        "latest_model.zip, or seedon_ppo_*_steps.zip under "
        f"{models_root}."
    )
...
def resolve_vecnorm_path(
    ...
    """Resolve the VecNormalize stats used with the Seedon policy."""
    ...
    if match:
        candidates.append(models_root / f"seedon_vecnorm_{match.group(1)}_steps.pkl")
...
def _load_train_config(path: Path, *, required: bool) -> dict | None:
    """Load a Seedon training config JSON object if available.
    ...
    if not path.is_file():
        if required:
            raise FileNotFoundError(f"Seedon train config not found: {path}")
        return None
    with path.open("r", encoding="utf-8") as handle:
        data = json.load(handle)
    if not isinstance(data, dict):
        raise ValueError(f"Seedon train config must be a JSON object: {path}")
    return data
...
def _load_reward_config_from_train_config(
    ...
    *,
    required: bool,
) -> SeedonStandingConfig | None:
    """Return ``SeedonStandingConfig`` from a saved training config."""
    ...
    if reward_config is None:
        if required:
            raise ValueError(f"Seedon train config has no reward_config: {train_config_path}")
        return None
    if not isinstance(reward_config, dict):
        raise ValueError(f"Seedon reward_config must be a JSON object: {train_config_path}")
    try:
        return SeedonStandingConfig(**reward_config)
    except TypeError as exc:
        raise ValueError(
            f"Seedon reward_config is incompatible with current env fields: {train_config_path}"
        ) from exc
...
def _resolve_scene_path(
    *,
    ...
    env_scene_path = os.environ.get("SEEDON_SCENE_PATH")
    if env_scene_path:
...
def _build_reward_config_with_safe_ranges(
    ...
    *,
    ...
    """Return the active Seedon config with optional eval-only knee-safe overrides."""
    if os.environ.get(CONFIG_OVERRIDES_ENV):
        config = load_seedon_config_from_env()
    elif ignore_train_config:
        config = load_seedon_config_from_env()
    else:
        config = (
            _load_reward_config_from_train_config(train_config_path, required=False)
            or load_seedon_config_from_env()
        )
...
def build_eval_vec_env(
    ...
    """Create a normalized Seedon eval environment."""
...
def evaluate_policy(
    ...
    *,
    ...
    fps: int = SeedonStandingEnv.metadata["render_fps"],
...
) -> list[SeedonEvalEpisode]:
    """Run deterministic Seedon policy evaluation episodes."""
    if episodes <= 0:
    ...
    results: list[SeedonEvalEpisode] = []
...
def write_csv(path: Path, episodes: list[SeedonEvalEpisode]) -> None:
...
def print_summary(episodes: list[SeedonEvalEpisode]) -> None:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse Seedon evaluation CLI arguments."""
    ...
    parser.add_argument(
        ...
        help="Directory containing Seedon model artifacts.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Training config used to restore Seedon reward/gait config and scene path. "
            "Defaults to logs/seedon/train_config.json."
        ),
    )
    parser.add_argument(
        "--ignore-train-config",
        action="store_true",
        help="Use env/default Seedon config instead of logs/seedon/train_config.json.",
    )
    parser.add_argument(
        "--scene-path",
        type=Path,
    ...
    parser.add_argument("--fps", type=int, default=SeedonStandingEnv.metadata["render_fps"])
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    """Evaluate a Seedon standing policy checkpoint."""
