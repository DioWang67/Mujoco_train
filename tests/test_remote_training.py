...
def test_env_project_value_prefers_project_specific_key() -> None:
    ...
    assert env_project_value(values, "seedon", "RESUME_N_ENVS", "4") == "128"
...
def test_remote_start_script_is_project_parameterized() -> None:
    ...
    assert "SEEDON_CONFIG_OVERRIDES" not in script
...
def test_remote_start_script_supports_fresh_config_run() -> None:
    script = remote_start_script(
        ...
        config_overrides="configs/seedon/reference.json",
        extra_args="",
        train_mode="fresh",
    )

    assert "Train mode: $TRAIN_MODE" in script
    assert "SEEDON_CONFIG_OVERRIDES=configs/seedon/reference.json" in script
    assert "--resume" not in script
    assert "fresh_seedon_" in script


def test_remote_status_script_filters_one_project() -> None:
    script = remote_status_script("/remote/root", "seedon")

    assert "[t]rain.py --project seedon" in script
    assert "/remote/root/runs/seedon/logs/seedon" in script
