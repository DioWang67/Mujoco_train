...
from tools.audit_seedon_teacher_imitation import build_parser
...
def test_teacher_audit_parser_accepts_random_residual_and_checkpoint_alias() -> None:
    args = build_parser().parse_args(
        [
            ...
            "--random-residual",
            "--checkpoint",
            "models/seedon/latest_model.zip",
            "--audit-warmup-steps",
            "20",
        ]
    )
    ...
    assert args.model_path == Path("models/seedon/latest_model.zip")
...
def test_random_residual_rejects_model_path() -> None:
    with pytest.raises(ValueError, match="random_residual"):
        audit_shuffle(
            Path("configs/seedon/reference_teacher_pose_1_4_imitation.json"),
            Path("models/seedon/latest_model.zip"),
            None,
            steps=1,
            seed=0,
