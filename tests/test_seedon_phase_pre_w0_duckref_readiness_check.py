...
from tools.seedon_phase_pre_w0_duckref_readiness_check import ReadinessConfig, build_summary
...
def write_required_inputs(config: ReadinessConfig, *, unknown_actuators: int = 0) -> None:
    ...
    write_json(
        config.m0_dir / "seedon_duck_scaled_gait_reference.json",
        {
            "seedon_target_vx": 0.141,
            "seedon_foot_clearance_target": 0.043,
            "seedon_action_scale_initial": 0.25,
        },
    )
    write_json(config.m0_dir / "seedon_joint_sign_mapping.json", {"R_joint_hip_pitch": {}})
    write_json(
    ...
    write_json(
        config.actuator_dir / "seedon_actuator_semantics_summary.json",
        {
    ...
    write_json(
        config.free_space_dir / "seedon_free_space_joint_sign_mapping.json",
        {
