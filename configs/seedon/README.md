...
Seedon configs are JSON overrides consumed through `SEEDON_CONFIG_OVERRIDES`.
They do not replace `configs/seedon/train.json`; they override reward, gait,
termination, and controller settings inside `SeedonStandingConfig`.
...
For Blue/BDX-style Seedon gait training right now:
...
For debugging why Seedon cannot unload a foot:
