...
SCENE_PATH = Path(os.environ.get("SEDON_SCENE_PATH", str(DEFAULT_SCENE_PATH))).expanduser()
...
def _make_env(seed: int, rank: int, reset_noise_scale: float):
    ...
    def _thunk():
        env = SedonStandingEnv(scene_path=SCENE_PATH, reset_noise_scale=reset_noise_scale)
