...
def load_reference_gait_seed(path: Path) -> ReferenceGaitSeed:
    ...
    Args:
        path: JSON seed path using the ``sedon_gait_seed.v1`` schema.
    ...
    if payload.get("schema") != "sedon_gait_seed.v1":
        raise ValueError(f"Unsupported reference gait seed schema: {path}")
    if tuple(payload.get("joint_names", [])) != JOINT_NAMES:
        raise ValueError(f"Reference gait seed joint_names do not match Sedon joints: {path}")
...
def compute_march_in_place_reward(
    *,
    ...
    """Compute reward terms for the staged Sedon march-in-place curriculum.
...
class SedonStandingEnv(MujocoEnv):
    ...
    def set_reward_config(self, reward_config: SedonStandingConfig) -> None:
